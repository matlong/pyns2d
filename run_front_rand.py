...
from svd_noise import svd_noise_2d 
...
front_type = 'singular' # 'singular' or 'nonsingular' or 'strong'
x = np.linspace(sqg.dx/2, sqg.Lx, sqg.nx, dtype=np.float64)
y = np.linspace(sqg.dy/2, sqg.Ly, sqg.ny, dtype=np.float64)
x, y = np.meshgrid(x, y, indexing='xy')    
if front_type == 'singular':
    b = np.sin(x) * np.sin(y) + np.cos(y)
if front_type == 'nonsingular':
    b = -(np.cos(2*x)*np.cos(y) + np.sin(x)*np.sin(y))
if front_type == 'strong':
    b = np.cos(2*x)*np.cos(y) + np.sin(x)*np.sin(y) + np.cos(2*x)*np.sin(3*y)

# Add SVD-type noise
param = {
        'n_obs': 21, # number of pseudo observations
        'n_ens': sqg.n_ens, # number of noise realizations
        'patch_dim': 3, # gird points in each direction of patch
        'pad_mode': 'wrap', # padding with boundary condition (see options in numpy.pad)
        'grid_size': [sqg.nx, sqg.ny], # grid points
        }
svdn = svd_noise_2d(param)
bnoi = svdn.draw_scalar_noise(b.T).transpose(0,2,1)
sqg.input_tracer(b + bnoi)
